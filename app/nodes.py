...
def _get_grader_chain():
    ...
    if _grader_chain is None:
        ...
        llm = _get_llm()  # Get LLM first
        grader_prompt = ChatPromptTemplate.from_template(GRADER_PROMPT)
        _grader_chain = grader_prompt | llm | StrOutputParser()
...
def _get_generator_chain():
    ...
    if _generator_chain is None:
        ...
        _generator_chain = generator_prompt | llm | StrOutputParser()
    return _generator_chain
...
def _get_brave_search():
    from app.tools import brave_search_results
...
def grade_documents(state):
    ...
    grades = []
    grader_chain = _get_grader_chain()
...
def web_search(state):
    ...
    brave_search_results = _get_brave_search()
    try:
...
def generate(state):
    ...
    if web_docs:
        ...
        if grades:
            relevant_ids = {g.doc_id for i, g in enumerate(grades) if g.grade == Grade.RELEVANT and g.score > 0.6}
    ...
    generator_chain = _get_generator_chain()
